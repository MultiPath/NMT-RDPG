...
def param_init_fflayer(options, params, prefix='ff', nin=None, nout=None,
                       ortho=True, negative=0, scale=0.01):
    if nin is None:
...
#
def param_init_gru(options, params, prefix='gru', nin=None, dim=None, scale=0.01):
    if nin is None:
        nin = options['dim_proj']
    if dim is None:
    ...
    W = numpy.concatenate([norm_weight(nin, dim, scale=scale),
    ...
    Wx = norm_weight(nin, dim, scale=scale)
...
def param_init_gru_cond(options, params, prefix='gru_cond',
                        ...
                        nin_nonlin=None, dim_nonlin=None, scale=0.01):
    if nin is None:
        nin = options['dim']
    if dim is None:
        dim = options['dim']
    ...
    Wx = norm_weight(nin_nonlin, dim_nonlin, scale=scale)
    params[_p(prefix, 'Wx')] = Wx
    ...
    # context to LSTM
    Wc = norm_weight(dimctx, dim*2, scale=scale)
    params[_p(prefix, 'Wc')] = Wc

    Wcx = norm_weight(dimctx, dim, scale=scale)
    params[_p(prefix, 'Wcx')] = Wcx

    # attention: combined -> hidden
    W_comb_att = norm_weight(dim, dimctx, scale=scale)
    params[_p(prefix, 'W_comb_att')] = W_comb_att

    # attention: context -> hidden
    Wc_att = norm_weight(dimctx, scale=scale)
    ...
    # attention:
    U_att = norm_weight(dimctx, 1, scale=scale)
...
# LN-GRU layer
def param_init_lngru(options, params, prefix='lngru', nin=None, dim=None, scale=0.01):
    """
