...
def return_reward(**_k):
    ...
    def LatencyBLEUex(beta=None, discount=1., return_quality=False):
        ...
        q     = numpy.zeros((_k['steps'],))
        lb    = numpy.zeros((_k['steps'],))
        ts    = numpy.zeros((_k['steps'],))
        ...
        if return_quality:  # instant reward sequence (Latency BLEU)
            return q


        # cumulitive futurereward (with discounting factor)
        ...
        if discount == 1:
            ...
            if FLAG:
                GAMMA  = compute_discount(gamma, maxlen)
                FLAG   = False

            R = numpy.dot(GAMMA[:L, :L], q[:, None]).flatten()
        # import sys
        # sys.exit(123)
        # print q  # collect all instant reward

        d = NormalizedDelay()
        return R, q[-1], d[-1], lbn[-1] + q[-1]
    ...
    def AwardForget(_max=5, beta=0.1):
        ...
        for it, a in enumerate(_k['act']):
            ...
            if ((_cur - _end) >= _max) and (a == 2):
                s[it] = 1
        return s * beta, _cur / float(_k['src_max'])
    ...
    def ReturnI():
        ...
        q[-1] = 0
        d     = MaximumDelay(_max=5, beta=beta)
        s, _  = AwardForget(_max=maxsrc, beta=0.01)
    ...
    def ReturnJ():
        ...
        q[-1] = 0
        d     = MaximumDelay(_max=5, beta=beta)
        # s, m  = AwardForget(_max=5, beta=0.01)

        r0    = q + d # + s
        rg    = bleu # * m  # it is a global reward, will not be discounted.
    ...
    funcs = [ReturnA, ReturnB, ReturnC, ReturnD, ReturnE, ReturnF, ReturnG, ReturnH, ReturnI, ReturnJ]
    return funcs[type]()

    # return FullQualityDelay(tau, gamma)
    # return QualityDelay(tau=tau, gamma=gamma)

    # return LatencyBLEUex()
    # return Q2D4(0.2)
    # return Q2Ds()
