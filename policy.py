...
class Controller(object):
    ...
    def build_sampler(self, options):
        ...
        if self.type == 'categorical':
            ...
            # add action mask
            if self.policy.get('act_mask', False):
                act_mask  = tensor.matrix('act_mask', dtype='float32')
                act_inps += [act_mask]
                act_prob *= act_mask
                act_prob /= (act_prob.sum(axis=-1, keepdims=True) + TINY)
                act_prob *= act_mask
    ...
    def build_discriminator(self, options):
        ...
        if self.type == 'categorical':
            ...
            if 'act_mask' in self.policy and self.policy['act_mask']:
                act_masks   = tensor.tensor3('act_masks', dtype='float32')
                act_inputs += [act_masks]
                act_probs  *= act_masks
                act_probs  /= (act_probs.sum(axis=-1, keepdims=True) + TINY)
                act_probs  *= act_masks
    ...
    def action(self, states, prevhidden, act_mask=None):
        if act_mask is None:
            return self.f_action(states, prevhidden)
        else:
            return self.f_action(states, prevhidden, act_mask)
    ...
    def build_advantages(self, act_inputs, reward, baseline, normalize=True):
        ...
        if normalize:
            reward_mean  = tensor.sum(mask * reward_adv) / (tensor.sum(mask) + TINY)
            reward_mean2 = tensor.sum(mask * (reward_adv ** 2)) / (tensor.sum(mask) + TINY)
            reward_std   = tensor.sqrt(tensor.maximum(reward_mean2 - reward_mean ** 2, TINY))
            # reward_std  = tensor.maximum(reward_std, 1)
            reward_c     = reward_adv - reward_mean  # independent mean
            advantages   = reward_c / (reward_std + TINY)
        else:
            advantages   = reward_adv

        print 'build advantages and baseline gradient'
        L      = tensor.sum(mask * (reward_adv ** 2)) / (tensor.sum(mask) + TINY)
        ...
        # f_adv, f_update_b = rmsprop(lr, self.tparams_b, dL, inps_b, oups_b)
    ...
    def build_reinforce(self, act_inputs, act_probs, actions, advantages):
        ...
        if self.type == 'categorical':
            if self.policy.get('act_mask', False):
                act_masks   = act_inputs[2]
                negEntropy  = tensor.sum(tensor.log(act_probs) * (act_probs * act_masks), axis=-1)
            else:
        ...
        # f_cost, f_update = rmsprop(lr, self.tparams, dJ, inps, outps)
    ...
    def run_reinforce(self, act_inputs, actions, reward, update=True, lr=0.0001):
        ...
        if self.type == 'gaussian':
            ...
            info = {'J': J, 'G_norm': H, 'B_loss': L, 'Adv': advantages.mean()}
